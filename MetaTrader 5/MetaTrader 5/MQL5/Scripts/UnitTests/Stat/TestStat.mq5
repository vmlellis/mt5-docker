...
//--- check calculated values
   for(int i=0;i<N;i++)
     {
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,binomial_pdf[i],binomial_cdf[i],binomial_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_binomial_pdf[i],delta_binomial_cdf[i],delta_binomial_quantile[i]);
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("2 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,binomial_pdf[i],binomial_cdf[i],binomial_quantile[i]);
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_binomial_pdf[i],delta_binomial_cdf[i],delta_binomial_quantile[i]);
      if(MathAbs(delta_binomial_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_binomial_cdf[i])>calc_precision_cdf)
         return false;
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("3 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,binomial_pdf[i],binomial_cdf[i],binomial_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_binomial_pdf[i],delta_binomial_cdf[i],delta_binomial_quantile[i]);
      if(MathAbs(delta_binomial_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_binomial_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomBinomial(n,probability,random_data_count,values_rnd)==true)
     {
      ...
      double mean=MathMean(values_rnd,0,random_data_count);
      double variance=MathVariance(values_rnd,0,random_data_count);
      double skewness=MathSkewness(values_rnd,0,random_data_count);
      double kurtosis=MathKurtosis(values_rnd,0,random_data_count);
      //--- inital values
      ...
      if(MathMomentsBinomial(n,probability,binomial_mean,binomial_variance,binomial_skewness,binomial_kurtosis,error_code))
        {
/*if(MathAbs(mean-binomial_mean)>0.5)
            return false;
         if(MathAbs(variance-binomial_variance)>0.5)
            return false;
         if(MathAbs(skewness-binomial_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-binomial_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,beta_pdf[i],beta_cdf[i],beta_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_beta_pdf[i],delta_beta_cdf[i],delta_beta_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,beta_pdf[i],beta_cdf[i],beta_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_beta_pdf[i],delta_beta_cdf[i],delta_beta_quantile[i]);
      if(MathAbs(delta_beta_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_beta_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomBeta(a,b,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsBeta(a,b,beta_mean,beta_variance,beta_skewness,beta_kurtosis,error_code))
        {
/*if(MathAbs(mean-beta_mean)>0.5)
            return false;
         if(MathAbs(variance-beta_variance)>0.5)
            return false;
         if(MathAbs(skewness-beta_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-beta_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   if(MathRandomGamma(a,b,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsGamma(a,b,gamma_mean,gamma_variance,gamma_skewness,gamma_kurtosis,error_code))
        {
/*if(MathAbs(mean-gamma_mean)>1.0)
            return false;
         if(MathAbs(variance-gamma_variance)>1.0)
            return false;
         if(MathAbs(skewness-gamma_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-gamma_kurtosis)>3.0)
            return false;*/
        }
      else
         return false;
   ...
   if(MathRandomExponential(mu,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsExponential(mu,exponential_mean,exponential_variance,exponential_skewness,exponential_kurtosis,error_code))
        {
         //PrintFormat("Exponential mean=%.10f, Exponential variance=%.10f  Exponential skewness=%.10f  Exponential kurtosis=%.10f",exponential_mean,exponential_variance,exponential_skewness,exponential_kurtosis);
...
/*if(MathAbs(mean-exponential_mean)>2.0)
            return false;
         if(MathAbs(variance-exponential_variance)>2.0)
            return false;
         if(MathAbs(skewness-exponential_skewness)>2.0)
            return false;
         if(MathAbs(kurtosis-exponential_kurtosis)>4.0)
            return false;*/
        }
      else
         return false;
   ...
   if(MathRandomUniform(a,b,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsUniform(a,b,uniform_mean,uniform_variance,uniform_skewness,uniform_kurtosis,error_code))
        {
/* if(MathAbs(mean-uniform_mean)>0.5)
            return false;
         if(MathAbs(variance-uniform_variance)>0.5)
            return false;
         if(MathAbs(skewness-uniform_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-uniform_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   if(MathRandomGeometric(p,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsGeometric(p,geometric_mean,geometric_variance,geometric_skewness,geometric_kurtosis,error_code))
        {
         //PrintFormat("Geometric mean=%.10f, Geometric variance=%.10f  Geometric skewness=%.10f  Geometric kurtosis=%.10f",geometric_mean,geometric_variance,geometric_skewness,geometric_kurtosis);
...
/*if(MathAbs(mean-geometric_mean)>2.0)
            return false;
         if(MathAbs(variance-geometric_variance)>2.0)
            return false;
         if(MathAbs(skewness-geometric_skewness)>2.0)
            return false;
         if(MathAbs(kurtosis-geometric_kurtosis)>2.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_hypergeometric_pdf[i],delta_hypergeometric_cdf[i],delta_hypergeometric_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
...
//--- calculate differences
   ...
   for(int i=0;i<N;i++)
     {
      //PrintFormat("2 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,hypergeometric_pdf[i],hypergeometric_cdf[i],hypergeometric_quantile[i]);
      delta_hypergeometric_pdf[i]=pdf_values[i]-hypergeometric_pdf[i];
      delta_hypergeometric_cdf[i]=cdf_values[i]-hypergeometric_cdf[i];
      delta_hypergeometric_quantile[i]=x_values[i]-hypergeometric_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_hypergeometric_pdf[i],delta_hypergeometric_cdf[i],delta_hypergeometric_quantile[i]);
     }
//--- calculate max error
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("3 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,hypergeometric_pdf[i],hypergeometric_cdf[i],hypergeometric_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_hypergeometric_pdf[i],delta_hypergeometric_cdf[i],delta_hypergeometric_quantile[i]);
      if(MathAbs(delta_hypergeometric_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_hypergeometric_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomHypergeometric(m,k,n,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsHypergeometric(m,k,n,hypergeometric_mean,hypergeometric_variance,hypergeometric_skewness,hypergeometric_kurtosis,error_code))
        {
         //PrintFormat("Hypergeometric mean=%.10f, Hypergeometric variance=%.10f  Hypergeometric skewness=%.10f  Hypergeometric kurtosis=%.10f",hypergeometric_mean,hypergeometric_variance,hypergeometric_skewness,hypergeometric_kurtosis);
...
/*if(MathAbs(mean-hypergeometric_mean)>1.0)
            return false;
         if(MathAbs(variance-hypergeometric_variance)>1.0)
            return false;
         if(MathAbs(skewness-hypergeometric_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-hypergeometric_kurtosis)>1.0)
            return false;*/
        }
      else
         return false;
   ...
   if(MathRandomLogistic(mu,sigma,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsLogistic(mu,sigma,logistic_mean,logistic_variance,logistic_skewness,logistic_kurtosis,error_code))
        {
/*if(MathAbs(mean-logistic_mean)>1.0)
            return false;
         if(MathAbs(variance-logistic_variance)>1.0)
            return false;
         if(MathAbs(skewness-logistic_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-logistic_kurtosis)>1.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_weibull_pdf[i],delta_weibull_cdf[i],delta_weibull_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_weibull_cdf[i]=cdf_values[i]-weibull_cdf[i];
      delta_weibull_quantile[i]=x_values[i]-weibull_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_weibull_pdf[i],delta_weibull_cdf[i],delta_weibull_quantile[i]);
     }
//--- calculate max error
   ...
   if(MathRandomWeibull(a,b,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsWeibull(a,b,weibull_mean,weibull_variance,weibull_skewness,weibull_kurtosis,error_code))
        {
         //PrintFormat("Weibull mean=%.10f, Weibull variance=%.10f  Weibull skewness=%.10f  Weibull kurtosis=%.10f",weibull_mean,weibull_variance,weibull_skewness,weibull_kurtosis);
...
/*if(MathAbs(mean-weibull_mean)>0.5)
            return false;
         if(MathAbs(variance-weibull_variance)>0.5)
            return false;
         if(MathAbs(skewness-weibull_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-weibull_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_poisson_pdf[i],delta_poisson_cdf[i],delta_poisson_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_poisson_cdf[i]=cdf_values[i]-poisson_cdf[i];
      delta_poisson_quantile[i]=x_values[i]-poisson_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_poisson_pdf[i],delta_poisson_cdf[i],delta_poisson_quantile[i]);
     }
//--- calculate max error
   ...
   if(MathRandomPoisson(lambda,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsPoisson(lambda,poisson_mean,poisson_variance,poisson_skewness,poisson_kurtosis,error_code))
        {
         //PrintFormat("Poisson mean=%.10f, Poisson variance=%.10f  Poisson skewness=%.10f  Poisson kurtosis=%.10f",poisson_mean,poisson_variance,poisson_skewness,poisson_kurtosis);
...
/*if(MathAbs(mean-poisson_mean)>1.0)
            return false;
         if(MathAbs(variance-poisson_variance)>1.0)
            return false;
         if(MathAbs(skewness-poisson_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-poisson_kurtosis)>1.0)
            return false;*/
        }
      else
         return false;
   ...
   if(MathRandomF(nu1,nu2,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsF(nu1,nu2,f_mean,f_variance,f_skewness,f_kurtosis,error_code))
        {
         //PrintFormat("F mean=%.10f, F variance=%.10f  F skewness=%.10f  F kurtosis=%.10f",f_mean,f_variance,f_skewness,f_kurtosis);
...
/*if(MathAbs(mean-f_mean)>2.0)
            return false;
         if(MathAbs(variance-f_variance)>2.0)
            return false;
         if(MathAbs(skewness-f_skewness)>2.0)
            return false;
         if(MathAbs(kurtosis-f_kurtosis)>10.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,chisquare_pdf[i],chisquare_cdf[i],chisquare_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_chisquare_pdf[i],delta_chisquare_cdf[i],delta_chisquare_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_chisquare_cdf[i]=cdf_values[i]-chisquare_cdf[i];
      delta_chisquare_quantile[i]=x_values[i]-chisquare_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_chisquare_pdf[i],delta_chisquare_cdf[i],delta_chisquare_quantile[i]);
     }
//--- calculate max error
   ...
   if(MathRandomChiSquare(nu,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsChiSquare(nu,chisquare_mean,chisquare_variance,chisquare_skewness,chisquare_kurtosis,error_code))
        {
         //PrintFormat("ChiSquare mean=%.10f, ChiSquare variance=%.10f  ChiSquare skewness=%.10f  ChiSquare kurtosis=%.10f",chisquare_mean,chisquare_variance,chisquare_skewness,chisquare_kurtosis);
...
/*if(MathAbs(mean-chisquare_mean)>1.0)
            return false;
         if(MathAbs(variance-chisquare_variance)>1.0)
            return false;
         if(MathAbs(skewness-chisquare_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-chisquare_kurtosis)>3.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_chisquare_pdf[i],delta_noncentral_chisquare_cdf[i],delta_noncentral_chisquare_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_noncentral_chisquare_cdf[i]=cdf_values[i]-noncentral_chisquare_cdf[i];
      delta_noncentral_chisquare_quantile[i]=x_values[i]-noncentral_chisquare_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_chisquare_pdf[i],delta_noncentral_chisquare_cdf[i],delta_noncentral_chisquare_quantile[i]);
     }
//--- calculate max error
   ...
   if(MathRandomNoncentralChiSquare(nu,sigma,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsNoncentralChiSquare(nu,sigma,noncentral_chisquare_mean,noncentral_chisquare_variance,noncentral_chisquare_skewness,noncentral_chisquare_kurtosis,error_code))
        {
         //PrintFormat("NoncentralChiSquare mean=%.10f, NoncentralChiSquare variance=%.10f  NoncentralChiSquare skewness=%.10f  NoncentralChiSquare kurtosis=%.10f",noncentral_chisquare_mean,noncentral_chisquare_variance,noncentral_chisquare_skewness,noncentral_chisquare_kurtosis);
...
/*if(MathAbs(mean-noncentral_chisquare_mean)>1.0)
            return false;
         if(MathAbs(variance-noncentral_chisquare_variance)>1.0)
            return false;
         if(MathAbs(skewness-noncentral_chisquare_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-noncentral_chisquare_kurtosis)>3.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_f_pdf[i],delta_noncentral_f_cdf[i],delta_noncentral_f_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_noncentral_f_cdf[i]=cdf_values[i]-noncentral_f_cdf[i];
      delta_noncentral_f_quantile[i]=x_values[i]-noncentral_f_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_f_pdf[i],delta_noncentral_f_cdf[i],delta_noncentral_f_quantile[i]);
     }
//--- calculate max error
   ...
   if(MathRandomNoncentralF(nu1,nu2,sigma,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsNoncentralF(nu1,nu2,sigma,noncentral_f_mean,noncentral_f_variance,noncentral_f_skewness,noncentral_f_kurtosis,error_code))
        {
         //PrintFormat("NoncentralF mean=%.10f, NoncentralF variance=%.10f  NoncentralF skewness=%.10f  NoncentralF kurtosis=%.10f",noncentral_f_mean,noncentral_f_variance,noncentral_f_skewness,noncentral_f_kurtosis);
...
/*if(MathAbs(mean-noncentral_f_mean)>2.0)
            return false;
         if(MathAbs(variance-noncentral_f_variance)>2.0)
            return false;
         if(MathAbs(skewness-noncentral_f_skewness)>2.0)
            return false;
         if(MathAbs(kurtosis-noncentral_f_kurtosis)>10.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,noncentral_beta_pdf[i],noncentral_beta_cdf[i],noncentral_beta_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_beta_pdf[i],delta_noncentral_beta_cdf[i],delta_noncentral_beta_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_noncentral_beta_cdf[i]=cdf_values[i]-noncentral_beta_cdf[i];
      delta_noncentral_beta_quantile[i]=x_values[i]-noncentral_beta_quantile[i];
      //PrintFormat("2 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,noncentral_beta_pdf[i],noncentral_beta_cdf[i],noncentral_beta_quantile[i]);
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_beta_pdf[i],delta_noncentral_beta_cdf[i],delta_noncentral_beta_quantile[i]);
     }
//--- calculate max error
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("3 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,noncentral_beta_pdf[i],noncentral_beta_cdf[i],noncentral_beta_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_beta_pdf[i],delta_noncentral_beta_cdf[i],delta_noncentral_beta_quantile[i]);
      if(MathAbs(delta_noncentral_beta_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_noncentral_beta_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomNoncentralBeta(a,b,lambda,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsNoncentralBeta(a,b,lambda,noncentral_beta_mean,noncentral_beta_variance,noncentral_beta_skewness,noncentral_beta_kurtosis,error_code))
        {
         //PrintFormat("NoncentralBeta mean=%.10f, NoncentralBeta variance=%.10f  NoncentralBeta skewness=%.10f  NoncentralBeta kurtosis=%.10f",noncentral_beta_mean,noncentral_beta_variance,noncentral_beta_skewness,noncentral_beta_kurtosis);
...
/*if(MathAbs(mean-noncentral_beta_mean)>1.0)
            return false;
         if(MathAbs(variance-noncentral_beta_variance)>1.0)
            return false;
         if(MathAbs(skewness-noncentral_beta_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-noncentral_beta_kurtosis)>1.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_negative_binomial_pdf[i],delta_negative_binomial_cdf[i],delta_negative_binomial_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_negative_binomial_cdf[i]=cdf_values[i]-negative_binomial_cdf[i];
      delta_negative_binomial_quantile[i]=x_values[i]-negative_binomial_quantile[i];
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_negative_binomial_pdf[i],delta_negative_binomial_cdf[i],delta_negative_binomial_quantile[i]);
     }
//--- calculate max error
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_negative_binomial_pdf[i],delta_negative_binomial_cdf[i],delta_negative_binomial_quantile[i]);
      if(MathAbs(delta_negative_binomial_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_negative_binomial_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomNegativeBinomial(r,p,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsNegativeBinomial(r,p,negative_binomial_mean,negative_binomial_variance,negative_binomial_skewness,negative_binomial_kurtosis,error_code))
        {
         //PrintFormat("NegativeBinomial mean=%.10f, NegativeBinomial variance=%.10f  NegativeBinomial skewness=%.10f  NegativeBinomial kurtosis=%.10f",negative_binomial_mean,negative_binomial_variance,negative_binomial_skewness,negative_binomial_kurtosis);
...
/*if(MathAbs(mean-negative_binomial_mean)>1.0)
            return false;
         if(MathAbs(variance-negative_binomial_variance)>1.0)
            return false;
         if(MathAbs(skewness-negative_binomial_skewness)>1.0)
            return false;
         if(MathAbs(kurtosis-negative_binomial_kurtosis)>1.0)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,t_pdf[i],t_cdf[i],t_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_t_pdf[i],delta_t_cdf[i],delta_t_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_t_cdf[i]=cdf_values[i]-t_cdf[i];
      delta_t_quantile[i]=x_values[i]-t_quantile[i];
      //PrintFormat("2 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,t_pdf[i],t_cdf[i],t_quantile[i]);
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_t_pdf[i],delta_t_cdf[i],delta_t_quantile[i]);
     }
//--- calculate max error
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("3 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,t_pdf[i],t_cdf[i],t_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_t_pdf[i],delta_t_cdf[i],delta_t_quantile[i]);
      if(MathAbs(delta_t_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_t_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomT(nu,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsT(nu,t_mean,t_variance,t_skewness,t_kurtosis,error_code))
        {
         //PrintFormat("T mean=%.10f, T variance=%.10f  T skewness=%.10f  T kurtosis=%.10f",t_mean,t_variance,t_skewness,t_kurtosis);
...
/*if(MathAbs(mean-t_mean)>0.5)
            return false;
         if(MathAbs(variance-t_variance)>0.5)
            return false;
         if(MathAbs(skewness-t_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-t_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,noncentral_t_pdf[i],noncentral_t_cdf[i],noncentral_t_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_t_pdf[i],delta_noncentral_t_cdf[i],delta_noncentral_t_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
   ...
   for(int i=0;i<N;i++)
     {
      ...
      delta_noncentral_t_cdf[i]=cdf_values[i]-noncentral_t_cdf[i];
      delta_noncentral_t_quantile[i]=x_values[i]-noncentral_t_quantile[i];
      //PrintFormat("2 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,noncentral_t_pdf[i],noncentral_t_cdf[i],noncentral_t_quantile[i]);
      //PrintFormat("2 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_t_pdf[i],delta_noncentral_t_cdf[i],delta_noncentral_t_quantile[i]);
     }
//--- calculate max error
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("3 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,noncentral_t_pdf[i],noncentral_t_cdf[i],noncentral_t_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_noncentral_t_pdf[i],delta_noncentral_t_cdf[i],delta_noncentral_t_quantile[i]);
      if(MathAbs(delta_noncentral_t_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_noncentral_t_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomNoncentralT(nu,delta,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsNoncentralT(nu,delta,noncentral_t_mean,noncentral_t_variance,noncentral_t_skewness,noncentral_t_kurtosis,error_code))
        {
         //PrintFormat("Noncentral T mean=%.10f, Noncentral T variance=%.10f  Noncentral T skewness=%.10f  Noncentral T kurtosis=%.10f",noncentral_t_mean,noncentral_t_variance,noncentral_t_skewness,noncentral_t_kurtosis);
...
/*if(MathAbs(mean-noncentral_t_mean)>0.5)
            return false;
         if(MathAbs(variance-noncentral_t_variance)>0.5)
            return false;
         if(MathAbs(skewness-noncentral_t_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-noncentral_t_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,normal_pdf[i],normal_cdf[i],normal_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_normal_pdf[i],delta_normal_cdf[i],delta_normal_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,normal_pdf[i],normal_cdf[i],normal_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_normal_pdf[i],delta_normal_cdf[i],delta_normal_quantile[i]);
      if(MathAbs(delta_normal_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_normal_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomNormal(mu,sigma,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsNormal(mu,sigma,normal_mean,normal_variance,normal_skewness,normal_kurtosis,error_code))
        {
/*if(MathAbs(mean-normal_mean)>0.5)
            return false;
         if(MathAbs(variance-normal_variance)>0.5)
            return false;
         if(MathAbs(skewness-normal_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-normal_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
   ...
   for(int i=0;i<N;i++)
     {
      ...
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,lognormal_pdf[i],lognormal_cdf[i],lognormal_quantile[i]);
      //PrintFormat("1 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_lognormal_pdf[i],delta_lognormal_cdf[i],delta_lognormal_quantile[i]);
     }
//--- check calculated values
   for(int i=0;i<N;i++)
     {
...
//--- check calculations
   for(int i=0;i<N;i++)
     {
      //PrintFormat("1 %d, PDF=%.20e, CDF=%.20e, Q=%.20e,",i,lognormal_pdf[i],lognormal_cdf[i],lognormal_quantile[i]);
      //PrintFormat("3 %d, dPDF=%.20e, dCDF=%.20e, dQ=%.20e,",i,delta_lognormal_pdf[i],delta_lognormal_cdf[i],delta_lognormal_quantile[i]);
      if(MathAbs(delta_lognormal_pdf[i])>calc_precision_pdf)
         return false;
      if(MathAbs(delta_lognormal_cdf[i])>calc_precision_cdf)
         return false;
   ...
   if(MathRandomLognormal(mu,sigma,random_data_count,values_rnd)==true)
     {
      ...
      if(MathMomentsLognormal(mu,sigma,lognormal_mean,lognormal_variance,lognormal_skewness,lognormal_kurtosis,error_code))
        {
         //PrintFormat("Lognormal mean=%.10f, Lognormal variance=%.10f  Lognormal skewness=%.10f  Lognormal kurtosis=%.10f",lognormal_mean,lognormal_variance,lognormal_skewness,lognormal_kurtosis);
...
/*if(MathAbs(mean-lognormal_mean)>0.5)
            return false;
         if(MathAbs(variance-lognormal_variance)>0.5)
            return false;
         if(MathAbs(skewness-lognormal_skewness)>0.5)
            return false;
         if(MathAbs(kurtosis-lognormal_kurtosis)>0.5)
            return false;*/
        }
      else
         return false;
