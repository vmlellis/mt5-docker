...
//--- generate normal and chisquare random variables 
   double rnd_normal=delta+x2*MathSqrt(-2.0*MathLog(r2)/r2);
   double rnd_nchi=MathRandomNoncentralChiSquare(nu,delta,error_code);
//--- calculate ratio
   double rnd_value=0;
   if(rnd_nchi!=0)
   ...
   for(int i=0; i<data_count; i++)
     {
      ...
      //--- generate normal and chisquare random variables 
      double rnd_normal=delta+x2*MathSqrt(-2.0*MathLog(r2)/r2);
      double rnd_nchi=MathRandomNoncentralChiSquare(nu,delta,err_code);
      //--- calculate ratio
      double rnd_value=0;
      if(rnd_nchi!=0)
        {
